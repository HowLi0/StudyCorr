...
namespace StudyCorr_GPU
{
    ...
    void StudyCorr_GPU::IO2D::saveTableCuda2D(std::vector<CudaPOI2D> &poi_queue)
    {
        std::ofstream file_out(file_path);
		file_out.setf(std::ios::fixed);
		file_out << std::setprecision(8);
		...
		if (file_out.is_open())
		{
			...
			for (std::vector<CudaPOI2D>::iterator iter = poi_queue.begin(); iter != poi_queue.end(); iter++)
			{
				file_out << iter->x << delimiter;
				file_out << iter->y << delimiter;
				...
				file_out << iter->subset_radius_x << delimiter;
				file_out << iter->subset_radius_y << delimiter;
				file_out << std::endl;
			}
		}
		file_out.close();
	...
	void IO2D::saveMap2D(std::vector<POI2D>& poi_queue, OutputVariable variable)
	{
		...
		int width = getWidth();
		cv::Mat output_map = cv::Mat::zeros(height, width, CV_32F);
		...
		case u:
			...
			{
				output_map.at<float>((int)poi_queue[i].y, (int)poi_queue[i].x) = poi_queue[i].deformation.u;
			}
			break;
		case v:
			for (int i = 0; i < poi_queue.size(); i++)
			{
				output_map.at<float>((int)poi_queue[i].y, (int)poi_queue[i].x) = poi_queue[i].deformation.v;
			}
			break;
		case zncc: //ZNCC value
			for (int i = 0; i < poi_queue.size(); i++)
			{
				output_map.at<float>((int)poi_queue[i].y, (int)poi_queue[i].x) = poi_queue[i].result.zncc;
			}
			break;
		case deformation_increment: //final ||delta_p||
			for (int i = 0; i < poi_queue.size(); i++)
			{
				output_map.at<float>((int)poi_queue[i].y, (int)poi_queue[i].x) = poi_queue[i].result.convergence;
			}
			break;
		case iteration_step: //iteration steps
			for (int i = 0; i < poi_queue.size(); i++)
			{
				output_map.at<float>((int)poi_queue[i].y, (int)poi_queue[i].x) = poi_queue[i].result.iteration;
			}
			break;
		case feature_nearby: //number of neighbor features
			for (int i = 0; i < poi_queue.size(); i++)
			{
				output_map.at<float>((int)poi_queue[i].y, (int)poi_queue[i].x) = poi_queue[i].result.feature;
			}
			break;
		case e_xx: //strain exx
			for (int i = 0; i < poi_queue.size(); i++)
			{
				output_map.at<float>((int)poi_queue[i].y, (int)poi_queue[i].x) = poi_queue[i].strain.exx;
			}
			break;
		case e_yy: //strain eyy
			for (int i = 0; i < poi_queue.size(); i++)
			{
				output_map.at<float>((int)poi_queue[i].y, (int)poi_queue[i].x) = poi_queue[i].strain.eyy;
			}
			break;
		case e_xy: //strain exy
			for (int i = 0; i < poi_queue.size(); i++)
			{
				output_map.at<float>((int)poi_queue[i].y, (int)poi_queue[i].x) = poi_queue[i].strain.exy;
			}
			break;
		default:
			return;
		...
		if (file_out.is_open())
		{
			for (int r = 0; r < height; r++)
			{
				...
				{
					file_out << output_map.at<float>(r, c) << delimiter;
				}
				file_out << std::endl;
			}
		}
	...
	void IO2D::saveMap2DS(std::vector<POI2DS>& poi_queue, OutputVariable  variable)
	{
		...
		case w:
			for (int i = 0; i < poi_queue.size(); i++)
			{
				output_map.at<float>((int)poi_queue[i].y, (int)poi_queue[i].x) = poi_queue[i].deformation.w;
			}
			break;
		case zncc_r1r2: //ZNCC value in matching between the two reference images
			for (int i = 0; i < poi_queue.size(); i++)
			{
				output_map.at<float>((int)poi_queue[i].y, (int)poi_queue[i].x) = poi_queue[i].result.r1r2_zncc;
			}
			break;
		case zncc: //ZNCC value in matching between the reference image and the target image from same view
			for (int i = 0; i < poi_queue.size(); i++)
			{
				output_map.at<float>((int)poi_queue[i].y, (int)poi_queue[i].x) = poi_queue[i].result.r1t1_zncc;
			}
			break;
		case zncc_r1t2: //ZNCC value in matching between the reference image and the target image from different views
			for (int i = 0; i < poi_queue.size(); i++)
			{
				output_map.at<float>((int)poi_queue[i].y, (int)poi_queue[i].x) = poi_queue[i].result.r1t2_zncc;
			}
			break;
		case e_xx: //strain exx
			for (int i = 0; i < poi_queue.size(); i++)
		...
		case e_zz: //strain ezz
			for (int i = 0; i < poi_queue.size(); i++)
			{
				output_map.at<float>((int)poi_queue[i].y, (int)poi_queue[i].x) = poi_queue[i].strain.ezz;
			}
			break;
		case e_xy: //strain exy
			for (int i = 0; i < poi_queue.size(); i++)
		...
		case e_yz: //strain eyz
			for (int i = 0; i < poi_queue.size(); i++)
			{
				output_map.at<float>((int)poi_queue[i].y, (int)poi_queue[i].x) = poi_queue[i].strain.eyz;
			}
			break;
		case e_zx: //strain ezx
			for (int i = 0; i < poi_queue.size(); i++)
			{
				output_map.at<float>((int)poi_queue[i].y, (int)poi_queue[i].x) = poi_queue[i].strain.ezx;
			}
			break;
		default:
			return;
